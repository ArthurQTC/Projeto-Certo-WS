...
import { 
  ...
  ArrowUpRight, Quote, Instagram, MessageCircle, X, Send,
  MapPin, Phone, Mail
} from 'lucide-react';
import React, { useState, useEffect } from 'react';
import { products, projects, team } from '../data';

const slides = [
  {
    image: 'https://architectural.hunterdouglas.com.br/wp-content/uploads/sites/5/slider/cache/002fa3605aa50e13f996283ea529bcc8/cinetic-brise-by-hunter-douglas-e-hans-donner-1.jpg',
    title: 'Cinetic Brise',
    subtitle: 'por Hunter Douglas & Hans Donner',
    description: 'O brise que é uma obra de arte — design e controle solar em perfeita harmonia',
    cta: 'Ver no Catálogo'
  },
  {
    image: 'https://architectural.hunterdouglas.com.br/bim/assets/img/Quiebravista_Quadrobrise_32x32.jpeg',
    title: 'QuadroBrise 32x32',
    subtitle: 'Brises de Controle Solar',
    description: 'Eficiência energética e elegância para fachadas residenciais e corporativas',
    cta: 'Conheça nossos brises'
  },
  {
    image: 'https://architectural.hunterdouglas.com.br/wp-content/uploads/sites/5/2025/03/Edificio-ON-Melo-Alves-Foto-Fran-Parente-compressed.jpg',
    title: 'Cortasol Fins',
    subtitle: 'Brises para Fachadas',
    description: 'Alta resistência, design elegante e controle preciso da luz natural',
    ...
    <div className="bg-white">
              ...
              >
                ...
                <AnimatePresence>
                  {isHovered && (
                    ...
                    >
                      <img
                        src="https://architectural.hunterdouglas.com.br/wp-content/themes/hunter-douglas/assets/img/hd-arch-logo-white.svg"
                ...
                {/* Content */}
                <div className="relative z-10 max-w-md transform transition-transform duration-500">
                  <span className="text-[10px] md:text-xs font-semibold text-brand-gold uppercase tracking-[0.3em] block mb-3">
                    {slide.subtitle}
                  </span>
                  
                  <h2 className="text-3xl xl:text-5xl font-light text-white uppercase tracking-tight leading-none mb-6">
                    {slide.title}
                  </h2>
                  
                  <p className={`text-xs xl:text-sm text-white/70 font-light tracking-wide leading-relaxed mb-8 transition-all duration-500 ${
                    isHovered ? 'opacity-100 max-h-32' : 'opacity-0 max-h-0 overflow-hidden'
                  }`}>
                    {slide.description}
                  </p>
                  
                  <button className="bg-brand-orange text-white px-8 py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-white hover:text-brand-black transition-all">
                    {slide.cta}
                  </button>
                </div>
              </div>
        ...
        <div className="lg:hidden h-full w-full relative">
          <div className="absolute inset-0 z-0">
            <AnimatePresence mode="wait">
              <motion.img
                key={activeHeroMobile}
                src={slides[activeHeroMobile].image}
                alt={slides[activeHeroMobile].title}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
          ...
          <div className="relative z-20 h-full flex flex-col justify-end p-8 pb-20">
            <div className="max-w-md">
              <span className="text-[10px] font-bold text-brand-gold uppercase tracking-[0.3em] block mb-2">
                {slides[activeHeroMobile].subtitle}
              </span>
              <h2 className="text-4xl font-light text-white uppercase tracking-tight leading-none mb-4">
                {slides[activeHeroMobile].title}
              </h2>
              <p className="text-sm text-white/70 font-light tracking-wide leading-relaxed mb-8">
                {slides[activeHeroMobile].description}
              </p>
              <button className="bg-brand-orange text-white px-8 py-4 text-xs font-bold uppercase tracking-[0.2em]">
                {slides[activeHeroMobile].cta}
              </button>
            </div>
      ...
      <section className="py-32 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <motion.span {...fadeIn} className="text-[11px] font-bold text-brand-gold uppercase tracking-[0.4em] block mb-8">
            Diferenciais
          </motion.span>
          <motion.h3 {...fadeIn} className="text-4xl md:text-5xl font-light tracking-tight mb-24">
            A excelência do controle solar
          </motion.h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            {[
              ...
              >
                <div className="w-20 h-20 bg-brand-black flex items-center justify-center text-brand-gold mb-8 group-hover:scale-110 transition-transform">
                  <f.icon className="w-8 h-8" />
                </div>
                <h4 className="text-lg font-medium tracking-widest uppercase mb-4">{f.title}</h4>
                <p className="text-gray-500 text-sm leading-relaxed max-w-xs">{f.text}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          <motion.div {...fadeIn}>
            ...
            <div className="space-y-6 text-gray-400 text-sm leading-relaxed max-w-xl font-light tracking-wide">
              ...
              <p>Como distribuidores autorizados Hunter Douglas no DF, Goiás e Tocantins, oferecemos toda a linha de brises originais.</p>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-end mb-32">
            <motion.div {...fadeIn}>
              ...
              <h3 className="text-4xl md:text-5xl font-light tracking-tight mb-8">Nossas Especialidades</h3>
            ...
            <div className="flex items-center space-x-12 border-l border-white/10 pl-12 h-32">
              <img src="https://architectural.hunterdouglas.com.br/wp-content/themes/hunter-douglas/assets/img/hd-arch-logo-white.svg" className="h-10 opacity-50" alt="HD Logo" />
              <div className="text-[10px] font-bold text-white/40 uppercase tracking-widest max-w-[150px]">Distribuidor Autorizado no DF, GO e TO</div>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { icon: HomeIcon, title: 'Residencial', text: 'Brises que controlam a entrada de luz e calor em casas e apartamentos, aliando conforto e sofisticação.', img: 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=600&q=80' },
              { icon: Building2, title: 'Corporativo', text: 'Fachadas com brises que reforçam a identidade visual dos edifícios e promovem eficiência energética.', img: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=600&q=80' },
              { icon: Layers, title: 'Comercial', text: 'Brises que valorizam a estética de lojas e shoppings, criando ambientes mais agradáveis e eficientes.', img: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=600&q=80' },
              { icon: Factory, title: 'Industrial', text: 'Coberturas e brises de alto desempenho para galpões, com foco em durabilidade e conforto térmico.', img: 'https://images.unsplash.com/photo-1565793298595-6a879b1d9492?w=600&q=80' }
            ].map((s, i) => (
              <motion.div
                key={i}
              ...
              >
                <img src={s.img} alt={s.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-110" />
                ...
                <div className="absolute inset-0 p-12 flex flex-col">
                  <div className="w-12 h-12 bg-brand-gold flex items-center justify-center mb-auto group-hover:scale-110 transition-transform">
                    <s.icon className="w-6 h-6 text-brand-black" />
                  </div>
                  <div className="mt-8">
                    <h4 className="text-xl font-light uppercase tracking-widest mb-4">{s.title}</h4>
                    <p className="text-white/50 text-[11px] font-light tracking-wide max-w-sm">{s.text}</p>
                  </div>
                </div>
                <div className="absolute top-12 right-12 opacity-0 group-hover:opacity-100 transition-all transform group-hover:rotate-45">
      ...
      {/* 8. PORTFOLIO */}
      <section id="projetos" className="py-32 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <motion.span {...fadeIn} className="text-[10px] font-bold text-brand-gold uppercase tracking-[0.4em] block mb-6">Portfolio</motion.span>
            <motion.h3 {...fadeIn} className="text-4xl md:text-5xl font-light tracking-tight mb-12">Nossos Projetos</motion.h3>
            <motion.div {...fadeIn} className="flex flex-wrap justify-center gap-4">
              {['Todos', 'Residencial', 'Comercial', 'Corporativo', 'Industrial'].map((f) => (
                <button
                  key={f}
                  onClick={() => setActiveFilter(f)}
                  className={`px-8 py-3 text-[10px] font-bold uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-brand-black text-white' : 'bg-white text-gray-400 hover:text-brand-black'}`}
                >
                  {f}
                </button>
              ))}
            ...
            <AnimatePresence mode="popLayout">
              {filteredProjects.map((p, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-brand-black/0 group-hover:bg-brand-black/80 transition-all duration-500 flex flex-col justify-center items-center text-center p-8 opacity-0 group-hover:opacity-100">
                    <p className="text-brand-gold text-[10px] font-bold uppercase tracking-[0.4em] mb-4">{p.category}</p>
                    <h4 className="text-xl font-light text-white tracking-widest uppercase mb-2">{p.title}</h4>
                    <p className="text-white/40 text-[10px] uppercase tracking-widest">{p.location}</p>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {team.map((member, i) => (
              ...
              >
                <img src={member.imageUrl} alt={member.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-brand-black to-transparent text-left transform translate-y-4 group-hover:translate-y-0 transition-transform">
                  ...
                  <div className="flex space-x-4 mt-6 opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                    {member.instagram && <Instagram className="w-4 h-4 text-white hover:text-brand-gold cursor-pointer" />}
                    {member.whatsapp && <MessageCircle className="w-4 h-4 text-white hover:text-brand-gold cursor-pointer" />}
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-32 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <motion.span {...fadeIn} className="text-[10px] font-bold text-brand-gold uppercase tracking-[0.4em] block mb-6">Depoimentos</motion.span>
            <motion.h3 {...fadeIn} className="text-4xl md:text-5xl font-light tracking-tight mb-12">O que dizem nossos clientes</motion.h3>
          </div>

          <div className="max-w-4xl mx-auto">
            <motion.div {...fadeIn} className="bg-white p-16 shadow-xl relative text-center">
              <Quote className="w-12 h-12 text-brand-gold/20 absolute -top-6 left-1/2 -translate-x-1/2" />
              <p className="text-2xl font-light tracking-wide leading-relaxed text-gray-700 mb-12 italic">
                "Preciso destacar a seriedade da equipe Projeto Certo. Me senti muito confortável com o compromisso e qualidade de entrega. É visível o quanto a Projeto Certo se importa com detalhes que fazem ABSOLUTA diferença."
              </p>
              <div className="flex flex-col items-center">
                <img src="https://images.unsplash.com/photo-1560250097-0b93528c311a?w=200&q=80" className="w-16 h-16 rounded-full grayscale mb-4 border-2 border-brand-gold p-1" alt="Marcelo Cruz" />
                <h4 className="text-xs font-bold uppercase tracking-widest">Marcelo Cruz</h4>
                <p className="text-[10px] text-brand-gold uppercase tracking-[0.2em] mt-1 font-bold">CEO do Grupo Referência</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section id="contato" className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-32">
          <motion.div {...fadeIn}>
            ...
            <a 
              href="https://wa.me/5562992952240"
              target="_blank"
            ...
            <form className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <input type="text" className="w-full bg-transparent border-b border-gray-200 py-3 outline-none focus:border-brand-gold transition-colors text-sm font-light" placeholder="Seu nome completo" />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Email</label>
                  <input type="email" className="w-full bg-transparent border-b border-gray-200 py-3 outline-none focus:border-brand-gold transition-colors text-sm font-light" placeholder="exemplo@email.com" />
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Assunto</label>
                <input type="text" className="w-full bg-transparent border-b border-gray-200 py-3 outline-none focus:border-brand-gold transition-colors text-sm font-light" placeholder="Ex: Orçamento de Brises" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Mensagem</label>
                <textarea className="w-full bg-transparent border-b border-gray-200 py-3 outline-none focus:border-brand-gold transition-colors text-sm font-light h-32 resize-none" placeholder="Conte-nos sobre seu projeto"></textarea>
              </div>
              <button className="w-full bg-brand-black text-white py-6 text-xs font-bold uppercase tracking-widest hover:bg-brand-gold transition-all duration-500">
                Enviar Mensagem
              </button>
            </form>
          </motion.div>
